...
def newsletter_signup(request):
    ...
    if form.is_valid():
        ...
        if NewsletterUser.objects.filter(email=instance.email).exists():
            messages.warning(request, 'Silahkan gunakan email yang belum berlangganan', "alert alert-warning start-dismissible")
        else:
            instance.save()
            messages.success(request, 'Email anda telah berhasil berlangganan', "alert alert-success alert-dismissible")
...
def newsletter_unsubscribe(request):
    ...
    if form.is_valid():
        ...
        if NewsletterUser.objects.filter(email=instance.email).exists():
            ...
            messages.success(request, 'Thanks for unsubscribe', 'alert alert-success alert-dismissible')
            subject = "Kamu telah berhenti Berlangganan"
            from_email = settings.EMAIL_HOST_USER
            to_email = [instance.email]
            ...
            messages.warning(request, 'Silahkan Subscribe terlebih dahulu', 'alert alert-warning alert-dimissible')
    ...
    if form.is_valid():
        ...
        if newsletter.status == "Publish":
            ...
            for email in newsletter.email.all():
                send_mail(subject=subject, form_email=form_email, recipient_list=[email], message=body, fail_silently=True)
